...
class SampleAssessment(db.Model):
    __tablename__ = 'sample_assessment'
...
class SampleAssessmentEnroll(db.Model):
    ...
    user_id = db.Column(db.Integer, db.ForeignKey('sample_users.id'))
    start_time = db.Column(db.DateTime, default=datetime.now(pytz.utc))
    finish_time = db.Column(db.DateTime)
    start_ip = db.Column(db.String(32))


class SampleAssessmentItems(db.Model):
    __tablename__ = 'sample_assessment_items'
