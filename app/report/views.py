...
def center():
    ...
    new_query = text("SELECT  * FROM CROSSTAB \
        ('select s.student_id, s.user_id, u.username, s.branch, ae.test_center, m.assessment_enroll_id, a2.name, \
        a2.id,t2.name, \
        ...
        group by s.student_id, s.user_id, u.username, a2.name, a2.id, s.branch, ae.test_center, m.assessment_enroll_id, \
        t2.name \
        order by s.student_id',\
        $$SELECT unnest(\'" + score_query + "\'::varchar[])$$) \
        AS ct(student_id VARCHAR ,user_id VARCHAR, username VARCHAR, branch VARCHAR, test_center VARCHAR, assessment_enroll_id integer, \
