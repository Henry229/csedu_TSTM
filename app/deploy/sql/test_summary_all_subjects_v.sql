...
AS WITH test_result_by_subject AS (
         SELECT test_summary_v.row_name[1] AS student_id,
           ...
           FROM crosstab('select ARRAY[student_id::integer, cs_student_id::integer, assessment_id::integer] as row_name, testset_id, my_score
						   from (SELECT m.student_id,
						          ...
						          WHERE m.student_id = s.user_id) test_summary_v
 ...
 SELECT trs.student_id,
  ...
  WHERE trs.assessment_id = a.assessment_id AND trs.student_id = a.student_id;
