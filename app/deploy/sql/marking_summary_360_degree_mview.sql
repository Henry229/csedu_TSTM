...
AS WITH marking_summary AS (
    SELECT e.id as assessment_enroll_id,
        m.testset_id,
        e.student_user_id,
        GROUPING(e.id, m.testset_id, e.student_user_id) AS "grouping",
    ...
    GROUP BY ROLLUP(e.id, m.testset_id, e.student_user_id)
    ORDER BY e.id, m.testset_id, e.student_user_id
    )
 SELECT marking_summary.assessment_enroll_id,
        (SELECT assessment_id FROM assessment_enroll WHERE id=marking_summary.assessment_enroll_id) AS "assessment_id",
    ...
    marking_summary.score,
        CASE
            WHEN marking_summary.total_score = 0::double precision THEN 0.0001::double precision
            ELSE marking_summary.total_score
        END AS total_score
   FROM marking_summary
  WHERE marking_summary.assessment_enroll_id IS NOT NULL
WITH DATA;
